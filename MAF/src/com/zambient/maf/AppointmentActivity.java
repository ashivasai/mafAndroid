...
public class AppointmentActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
   ...
   private  EditText time;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        setContentView(R.layout.content_appointment);
        ...
        new GetAppointmentDetails().execute(AppConstants.GETAPPOINTMENTDETAILS+"businessId="+pref.getString("bussinessId",null)+"&templateId="+pref.getString("templateId",null)+"&featureId=4&userId="+pref.getString("userId",null));
        new GetDoctorsList().execute(AppConstants.GETDOCTORSLISTURL+"businessId="+pref.getString("bussinessId",null)+"&templateId="+pref.getString("templateId",null)+"&featureId=11&userId="+pref.getString("userId",null));
        }else{
    ...
    private void validate(EditText name, EditText contactno, EditText mailid, EditText date, EditText time) {
        if(!(selectedItem!=null && !selectedItem.trim().matches("Select Doctor"))){
            Utils.toast("Please Select Doctor",context);
            ...
            try {
                review.accumulate("doctorName",doctorName);
                review.accumulate("specialization",specialisation);
                review.accumulate("patientName",name.getText().toString());
                review.accumulate("patientEmail",mailid.getText().toString());
                review.accumulate("mobileNumber",contactno.getText().toString());
                ...
                reviewparams.accumulate("bookAppointment", review);
            ...
            if(Utils.isNotNull(reviewparams.toString()))
                new SubmitAppointment(AppointmentActivity.this,context).execute(reviewparams.toString());
            }else{
                Utils.okButtonAlertDialog(AppConstants.NETWORK_MSG, context);
            }
   ...
   private void dateValidate(String date) throws ParseException {
        ...
        protected void onPostExecute(String  response) {
            ...
            if (response != null) {
                ...
                if (doctorsListDto.androidDoctorsList.doctorsList.size() != 0 && doctorsListDto.androidDoctorsList.doctorsList != null) {
                    ...
                    ArrayAdapter dataAdapter = new ArrayAdapter(AppointmentActivity.this, R.layout.spinner_item, expertsList);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        if(position!=0) {
            ...
            doctorName = doctorsListDto.androidDoctorsList.doctorsList.get(position - 1).name;
